...
const expenseCategoriesService = require('../services/expense-categories.service');
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (req.query.active !== undefined && req.query.active !== '') {
      active = expenseCategoriesService.parseActiveQuery(req.query.active);
    }
    const rows = await expenseCategoriesService.listExpenseCategories({
...
router.get('/:id', async (req, res, next) => {
  try {
    const row = await expenseCategoriesService.getExpenseCategoryById({
      id: req.params.id,
      clientId: req.user.clientId,
    });
...
router.post('/', requireCsrf, async (req, res, next) => {
  try {
    const row = await expenseCategoriesService.createExpenseCategory({
      clientId: req.user.clientId,
      userId: req.user.id,
      body: req.body || {},
    });
...
router.patch('/:id', requireCsrf, async (req, res, next) => {
  try {
    const row = await expenseCategoriesService.updateExpenseCategory({
      id: req.params.id,
      clientId: req.user.clientId,
      userId: req.user.id,
      body: req.body || {},
...
router.patch('/:id/active', requireCsrf, async (req, res, next) => {
  ...
  try {
    const row = await expenseCategoriesService.setExpenseCategoryActive({
      id: req.params.id,
      clientId: req.user.clientId,
      userId: req.user.id,
